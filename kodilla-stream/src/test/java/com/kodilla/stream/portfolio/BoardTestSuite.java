...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        int daysSpentOnAllTasks = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> Period.between(t.getCreated(), LocalDate.now()).getDays())
                .reduce(0, (sum, current) -> sum += current);

        int numberOfAllTasks = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(t -> Period.between(t.getCreated(), LocalDate.now()).getDays())
                .map(t -> 1)
        ...
        double retrievedAverage = daysSpentOnAllTasks / numberOfAllTasks;



        //Then
        Assert.assertEquals(10.0, retrievedAverage, 0.001);
    }
